...
class GWHP : public i2c::hw::BufferedSlave {
  ...
  struct {
    rgbLed::Setting test = {.rampUpTime = 0,
                            .onTime = 255,
                            .rampDownTime = 0,
                            .offTime = 0,
                            .rgb = {255, 255, 255}};
    rgbLed::Setting ready = {.rampUpTime = 150,
                             .onTime = 0,
                             .rampDownTime = 5,
                             .offTime = 10,
                             .rgb = {220, 255, 0}};
...
public:
  ...
  void init(int i2cAddress) {
    ...
    inputPins[2].init(&target::GPIOA, pwrOkPin, false, true);
    inputPins[3].init((volatile target::gpio_a::Peripheral *)&target::GPIOF, 0,
                      false, true); // LPS
    inputPins[4].init((volatile target::gpio_a::Peripheral *)&target::GPIOF, 1,
                      false, true); // HPS
    ...
    // RGB LED
    rgbLed.init(&target::GPIOB, 3, 1, 0, &target::TIM16, &rgbSettings.test);

    for (volatile int c = 0; c < 100000; c++)
      ;

    rgbLed.set(&rgbSettings.ready);
    ...
    BufferedSlave::init(&target::I2C1, i2cAddress,
                        (unsigned char *)&i2cRxBuffer, sizeof(i2cRxBuffer),
                        (unsigned char *)&i2cTxBuffer, sizeof(i2cTxBuffer));
    ...
    if (!read) {
      ...
      case CMD_SET_OUTPUT: {
        if (i2cRxBuffer.setOutput.outputIndex < outputPinsCount) {
          outputPins[i2cRxBuffer.setOutput.outputIndex].set(
              i2cRxBuffer.setOutput.state);
        }
        break;
      }
      case CMD_SET_EEV: {
...
void interruptHandlerI2C1() { gwhp.handleInterrupt(); }

void interruptHandlerTIM16() { gwhp.rgbLed.handleInterrupt(); }

void interruptHandlerTIM17() { gwhp.eev.handleInterrupt(); }

void interruptHandlerEXTI4_15() { gwhp.handlePwrOkInterrupt(); }
